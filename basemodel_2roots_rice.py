...
indic1L = Expression('0.5*(1+ (2/ppi)*atan((x[1] +(Len0 + G*t))/epss ))', degree=6, Len0=L_0, ppi = π, epss = ε_s, G=G, t=0)
indic1R = Expression('0.5*(1+ (2/ppi)*atan((x[1] +(Len0 + G*t))/epss ))', degree=6, Len0=L_0, ppi = π, epss = ε_s, G=G, t=0)
...
a2 = (θ + bx/(1 + kbx*YL_n)) * X*v_1*dx - (kbx*bx /((1 + kbx*YL_n)**2))* (YLs - YL_n) * X* v_1*dx + dt * dot(Dx * grad(X),grad(v_1)) * dx + dt * indic1L*α *X *v_1*ds(1) + dt * indic1R*α *X *v_1*ds(3)
...
file_flux_left = open('flux_left.txt', 'w')
file_flux_right = open('flux_right.txt', 'w')
